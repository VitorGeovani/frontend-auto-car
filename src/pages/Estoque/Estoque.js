...
const Estoque = () => {
  ...
  const aplicarFiltros = () => {
    ...
    if (filtros.anoMin) {
      carrosFiltrados = carrosFiltrados.filter(
        (carro) => carro.ano >= parseInt(filtros.anoMin)
      );
    }

    if (filtros.anoMax) {
      carrosFiltrados = carrosFiltrados.filter(
        (carro) => carro.ano <= parseInt(filtros.anoMax)
      );
    }
    ...
    if (filtros.kmMin) {
      carrosFiltrados = carrosFiltrados.filter(
        (carro) => (carro.quilometragem || 0) >= parseInt(filtros.kmMin)
      );
    }

    if (filtros.kmMax) {
      carrosFiltrados = carrosFiltrados.filter(
        (carro) => (carro.quilometragem || 0) <= parseInt(filtros.kmMax)
      );
    }
    ...
    if (filtros.combustivel) {
      carrosFiltrados = carrosFiltrados.filter(
        (carro) => carro.combustivel === filtros.combustivel
      );
    }
  ...
  const marcasDisponiveis = [
    ...new Set(carrosEstoque.map((carro) => carro.marca)),
  ];

  // Lista única de combustíveis para o filtro
  const combustiveisDisponiveis = [
    ...new Set(carrosEstoque.map((carro) => carro.combustivel).filter(Boolean)),
  ];
    ...
    <div className="estoque-page">
      <Container>
        ...
        <Row className="estoque-container">
          ...
          <Col
            md={3}
            className={`filtros-panel ${filtrosAbertos ? "aberto" : ""}`}
          >
          ...
          <Col md={9} className="estoque-list">
            {loading ? (
              ...
              <div className="empty-message">
                ...
                <p>
                  Não há veículos que correspondam aos critérios selecionados.
                </p>
              ...
              <>
                <div className="resultados-header">
                  ...
                  <p>
                    <strong>{carrosFiltrados.length}</strong>{" "}
                      ...
                      <Card className="carro-card">
                        ...
                        <Card.Body>
                          ...
                          <div className="carro-badges">
                            ...
                            <Badge bg="light" text="dark">
                              <FaTachometerAlt />{" "}
                              {(carro.quilometragem || 0).toLocaleString()} km
                            </Badge>
                            <Badge bg="light" text="dark">
                          ...
                          <div className="carro-detalhes">
                            ...
                            {carro.transmissao && (
                              <div className="detalhe-item">
                                <FaCog />{" "}
                                <span>Câmbio: {carro.transmissao}</span>
                              </div>
                            )}
                          </div>
