  ...
  return (
    <div className="admin-interesses">
      <div className="page-header">
        <h1>
          <FaBell className="icon" /> Gerenciamento de Interesses
        </h1>
        <button onClick={fetchInteresses} className="refresh-button">
          Atualizar
        </button>
      </div>
            ...
            <table className="interesses-table">
              ...
              <tbody>
                {interesses.map((interesse) => (
                  <tr
                    key={interesse.id}
                    className={interesse.lido ? "" : "unread"}
                  >
                    <td>{interesse.id}</td>
                    <td>
                      {new Date(interesse.data_registro).toLocaleDateString(
                        "pt-BR"
                      )}
                      <div className="time">
                        {new Date(interesse.data_registro).toLocaleTimeString(
                          "pt-BR",
                          {
                            hour: "2-digit",
                            minute: "2-digit",
                    ...
                    <td>
                      <div className="actions">
                        <Link
                          to={`/admin/interesses/${interesse.id}`}
