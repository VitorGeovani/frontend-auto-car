...
const EstoqueList = () => {
  ...
  const excluirItem = async (id, carroId) => {
    if (
      !window.confirm("Tem certeza que deseja remover este veículo do estoque?")
    ) {
      return;
    }
    ...
    try {
      ...
      if (confirmaExclusaoCompleta) {
        try {
          console.log(
            `Tentando excluir veículo ID: ${carroId} permanentemente...`
          );

          // a. PRIMEIRO: Excluir todos os interesses relacionados ao veículo
          try {
            console.log(`Excluindo interesses do veículo ${carroId}`);
            const interessesResponse = await api.get(
              `/interesses/carro/${carroId}`
            );

            if (interessesResponse?.data?.length > 0) {
              console.log(
                `Encontrados ${interessesResponse.data.length} interesses para excluir`
              );
            ...
            throw new Error(
              `Não foi possível excluir os interesses associados a este veículo.`
            );
          }
        ...
        toast.info(
          "Veículo removido apenas do estoque, mas mantido no sistema."
        );
      }
          ...
          <table className="estoque-table">
            ...
            <tbody>
              {estoque.map((item) => (
                <tr
                  key={item.id}
                  className={item.quantidade <= 0 ? "zero-stock" : ""}
                >
                  <td>{item.id}</td>
                  <td>
                    {item.modelo ||
                  ...
                  <td>
                    <div className="quantity-control">
                      ...
                      <span
                        className={item.quantidade <= 0 ? "zero-quantity" : ""}
                      >
                        {item.quantidade}
                      </span>
                      <button
